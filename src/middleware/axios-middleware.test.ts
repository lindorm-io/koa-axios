...
describe("axiosMiddleware", () => {
  ...
  test("should create axios client on context", async () => {
    await expect(axiosMiddleware(middlewareOptions)()(ctx, next)).resolves.toBeUndefined();
  ...
  test("should use path to set base url", async () => {
    await expect(axiosMiddleware(middlewareOptions)(options)(ctx, next)).resolves.toBeUndefined();
